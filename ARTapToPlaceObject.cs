...
public class ARTapToPlaceObject : MonoBehaviour
{
    ...
    private void PlaceObject()
    {
        ...
        //     Destroy(Cube);
        // }
        objectToPlace.SetActive(true); // false to hide, true to show
        objectToPlace.transform.SetPositionAndRotation(placementPose.position, placementPose.rotation);
        cubeText = objectToPlace.GetComponentInChildren<Text>();
        if (uiText.text == "")
        {
            cubeText.text = "Enter text...";
        }
        else
        {
            cubeText.text = uiText.text;
        }
        statusText.text = "";
        // exists = true;
    }
